...
@SessionAttributes("MemberVO") // request 공유 영역에 올린 객체 중, MemberVO 이름을 가진 객체는 Sesession 영역에 올려주는 어노테이션
@Controller
public class MemberController {
	...
	public String login(MemberVO member , Model model, HttpSession session) {
		...
		session.setAttribute("MemberVO", MemberVO); // redirect로 인해, session에 미리 등록해준다.
...
//		session.invalidate(); // @SessionAttributes 시 ,사용 불가
	...
	public String join(MemberVO member) {
		...
		String en_pwd = pwdEncoder.encode(member.getPwd());
		System.out.println(en_pwd);
		
		member.setPwd(en_pwd);
